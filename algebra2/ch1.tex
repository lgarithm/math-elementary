...
Let $X$ be a set, the \idx{power set} of $X$ is the set of all subsets of $X$.
\end{defi}

Power set of $X$ are denoted by $\mathcal P(X)$ or $2^X$.

\begin{defi}[cover]
Let $\mathcal C \subseteq \mathcal P(X)$, $\mathcal C$ is a \idx{cover} of $X$ iff
$$\bigcup_{U \in \mathcal C} U = X$$
\end{defi}

\begin{defi}[partition]
A cover $\mathcal C$ of $X$ is a \idx{partition} iff any two distinct elements of $\mathcal C$
are disjoint.
\end{defi}

\subsection{Relation}
\begin{defi}[binary relation]
Let $X, Y$ be sets, a \idx{binary relation} $R$ between $X$ and $Y$ is a subset of $X \times Y$.
\end{defi}
...
A binary relation $\sim$ on $X$ is an \idx{equivalent relation} iff it is transitive,
...
$O(x) = \{y \mid y \sim x\}$.
\end{defi}
An orbit is also called an \idx{equivalent class}.

\begin{defi}[quotient]
Let $\sim$ be an equivalent relation on $X$, the set of all orbits forms a partition
of $X$, called the \idx{quotient set} of $X$ modulo $\sim$, denoted by $X / \sim$.
\end{defi}
...
Let $X, Y, Z$ be sets, a \idx{binary operation} on $X, Y$ is a map $o : X \times Y \to Z$.
\end{defi}
